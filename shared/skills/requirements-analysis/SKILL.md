...
allowed-tools: Read, Write, Glob, Grep, Bash(git:*), Bash(gh:*), Bash(az:*), Bash(curl:*), Bash(jq:*), Bash(yq:*), Bash(shared/scripts/*)
---
...
Read (in order):
1. `shared/project/PROFILE.md` — if missing, abort with: `PROFILE.md not found. Run /bootstrap-project first.`
2. `shared/project/CLAUDE.md` (if present)
3. `shared/project/overrides/requirements-analysis.md` (if present)
4. `shared/project/guidelines/*.md` — all files
5. `shared/project/stacks/*.md` — only those matching the PROFILE
...
```bash
shared/scripts/ado-wi-show.sh $1 > /tmp/wi-$1.json
```
...
```bash
shared/scripts/ado-wi-update.sh $1 --discussion "Open question: <Question> → <who to ask>"
```

Then run:

```bash
shared/scripts/check-tech-agnostic.sh docs/designs/WI-$1/functional.md
```
...
shared/scripts/ado-wi-create-slice.sh $1 /tmp/slice-<id>.yaml
```
...
### Reviewers
<from CODEOWNERS or shared/project/PROFILE.md reviewers field>
EOF
)" \
  --work-items $1 \
  --output json
...
```bash
shared/scripts/slack-notify.sh "Design ready for review: WI-$1 — <PR URL>"
```
