...
allowed-tools: Task, Read, Write, Glob, Grep, Bash(git:*), Bash(gh:*), Bash(az:*), Bash(curl:*), Bash(jq:*), Bash(yq:*), Bash(shared/scripts/*)
---
...
1. Read `shared/project/PROFILE.md`, `CLAUDE.md`, `overrides/implement-slice.md` (applies to orchestrator too), `guidelines/*`, relevant `stacks/*`.
...
2. **Compile checks:** `shared/scripts/check-compile.sh both`
3. **Startup check:** `shared/scripts/check-startup.sh both`
...
BASE_URL=$(yq -r '.gates.healthcheck.frontend' shared/project/PROFILE.md \
...
```bash
shared/scripts/ado-wi-update.sh <slice-wi-id> --state "Resolved" \
  --discussion "Implemented in PR <feature-pr-url>"
```

```bash
shared/scripts/slack-notify.sh "Feature PR open for WI-$1 — <PR URL>"
```
